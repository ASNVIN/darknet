...
//	for (j = 0; j < l.w*l.h*l.n; ++j) detector_gpu.probs[j] = (float *)calloc(l.classes, sizeof(float));
	for (j = 0; j < l.w*l.h*l.n; ++j) detector_gpu.probs[j] = (float *)calloc(l.classes + 1, sizeof(float));
...
//	free(detector_gpu.probs);//可以用下面的一行替代
	free_ptrs((void **)detector_gpu.probs, l.w*l.h*l.n);
...
YOLODLL_API std::vector<bbox_t> Detector::detect(image_t img, float thresh)
{
	...
	image sized = letterbox_image(im, net.w, net.h);//changed from resize_image
...
//	get_region_boxes(l, 1, 1, thresh, detector_gpu.probs, detector_gpu.boxes, 0, 0);
	get_region_boxes(l, im.w, im.h, net.w, net.h, thresh, detector_gpu.probs, detector_gpu.boxes, 0, 0, 0, .5, 1);
